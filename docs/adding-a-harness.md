...
**instruction-tier** — the host reads a single instructions file and nothing else. It gets the instruction-tier ruleset (`skills/using-razorback/references/instruction-tier.md`) as a synced host copy, no skills and no delegation. Current: Copilot CLI (`.github/copilot-instructions.md`), plus `.clinerules/`, `.cursor/rules/`, `.kiro/steering/`, `.windsurf/rules/`.
