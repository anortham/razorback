...
## Why fresh subagents (not existing implementation-phase workers)

The external review runs after the implementation phase has ended. Fresh subagents work at any point in the timeline regardless of worker state. They also come with no implementation-phase bias that might rationalize around a finding - the implementer who wrote the code is the one most likely to explain away a real defect in it.
