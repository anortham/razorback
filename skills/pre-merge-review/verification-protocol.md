...
- `julie:deep_dive(symbol=<the referenced symbol>)` — confirm the symbol exists, check its callers/callees/types to see if the described path is real.
- `julie:fast_refs(symbol=<the referenced symbol>)` — if the finding touches a public API or shared utility, check the full reference graph to size the impact.
- `julie:get_symbols(file_path=<file>)` — scan the file's structure to confirm the reviewer is pointing at the right region.
...
>
> Lead verifies:
> - `julie:deep_dive(symbol='refreshToken')` — confirms the function mutates `session.expiresAt` at line 47, calls `validateToken()` at line 71, and returns early on validation failure without rolling back the mutation.
> - `julie:fast_refs(symbol='session.expiresAt')` — confirms `expiresAt` is read by middleware `requireAuth` at `src/middleware/auth.ts:22` for "is session still valid?" checks.
...
**Verify with Julie:** same tools as real-bug. The goal is to confirm the described improvement is applicable to the code as written, not to a hypothetical version of it.
...
>
> Lead verifies:
> - `julie:get_symbols(file_path='src/review/parse.ts', target='parseReviewOutput')` — confirms the helper catches `Error` at line 34 and throws `new Error('failed to parse review output: ' + err.message)` without preserving cause.
> - `julie:fast_refs(symbol='parseReviewOutput')` — 3 callers, all inside the review flow, all log the thrown error.
...
**Verify with Julie:** verify by checking the actual code. The standard of evidence is "I looked and the reviewer's premise is wrong" — not "I don't think that's a problem."
...
>
> Lead verifies:
> - `julie:get_symbols(file_path='src/config/load.ts', target='loadConfig')` — reads the function. The `readFileSync` call is inside a `try { ... } catch (err) { ... }` block that starts at line 80 and closes at line 110. The reviewer missed the outer try.
...
**Verify with Julie:** confirm the finding is real and well-grounded (same verification as real-bug / real-improvement), then compare against the plan's stated scope.
...
>
> Lead verifies:
> - `julie:deep_dive(symbol='exportReport')` — confirms the interpolation is real at lines 128, 141, 154.
> - `julie:fast_refs(symbol='exportReport')` — the callers pass user-sourced `reportId`, so the risk is real.
