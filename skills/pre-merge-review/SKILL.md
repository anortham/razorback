...
- The chosen reviewer's provider (`codex` → `openai`, `claude` → `anthropic`) passes the external-model policy check in razorback:security-review
...
Before the diff is sent, apply the external-model policy check in razorback:security-review to the dispatched CLI's provider (`codex` → `openai`, `claude` → `anthropic`); a denied provider means refuse this reviewer and name an allowed alternative.
