...
2. **Write one self-contained prompt per agent** — paste the error messages and test names. Each prompt includes:
   ...
   - Miller directives: orient with `context(query='<area>')`; `inspect(target='<fn>', depth=full)` before modifying a symbol; `trace(target='<symbol>')` before changing anything callers see; `inspect(target='<file>')` before reading a file; prove API shapes (symbol names, signatures, config shapes, routes, CLI flags, public contracts) with Miller evidence; no Glob → Read → Grep chains.
