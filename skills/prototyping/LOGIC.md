...
2. **Match the host project.** Its language, runtime, and task runner; no new package managers. Orient with code-kb (`codebase_outline` on the area, `file_skeleton` or `get_context_slice` on the neighboring module) so labels use the project's domain vocabulary. The HTML shell is the one language exception; the vocabulary rule still applies.
