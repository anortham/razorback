...
code-kb: `get_symbol_context` or `get_symbol_body` on the slow function; `find_references(symbol_name, direction="callers")` for every call site reaching the hot path; `codebase_outline` to orient; `blast_radius` before the fix.
